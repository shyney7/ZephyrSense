...
if (Test-Path $SummaryScript) {
    ...
    $SummaryArgs = @($SummaryScript, $ExportJson, $SummaryJson)

    # Append QML coverage if available (generated by run_qml_coverage.ps1)
    $QmlCoverageXml = Join-Path $CoverageDir "qml-coverage.xml"
    if (Test-Path $QmlCoverageXml) {
        $SummaryArgs += @("--qml", $QmlCoverageXml)
        Write-Host "Including QML coverage from $QmlCoverageXml" -ForegroundColor Cyan
    }
